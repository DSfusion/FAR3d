...
!		Only one energetic particle species	
		  ...
		  if(DIIID_u .eq. 0) then		  
            do i=1,ns0
	          read(nunit,*) rho_e(i),qprof(i),den_beam_e(i),den_ion_e(i),den_elec_e(i), &
                            ...
                            pres_thermal_e(i),pres_equil_e(i),pol_rot_vel_e(i),tor_rot_vel_e(i)       
  	        end do    
	        close(unit=nunit)
		  end if
	  ...
	  kblotz = 1.602e-19								  !! Conversion factor eV to J
          ...
          do je=0,mj
			...
			vAlfven(je) = (B0_e/sqrt(uion_e*mu0prof*dninn(je)*1.672e-27))/va0
          end do  	
          ...
          do je=0,mj
		    ...
		    vtherm_ionP(je) = sqrt(tinn(je)*1.6e-19/(uion_e*1.672e-27))/va0			
          end do	
         ...
         if(DIIID_u .eq. 0) then
          ...
          do je=0,mj
            xx = r(je)
            vzt_eqp(je) = seval(ns0,xx,rho_e,tor_rot_vel_e,bspl,cspl,dspl)*1.e+3/va0
          end do 				  
          call spline(ns0,rho_e,pol_rot_vel_e,bspl,cspl,dspl)
          do je=0,mj
            xx = r(je)
            vth_eqp(je) = seval(ns0,xx,rho_e,pol_rot_vel_e,bspl,cspl,dspl)*1.e+3/va0
          end do
	   
         else if(DIIID_u .eq. 1) then
          call spline(ns0,rho_e,tor_rot_vel_e,bspl,cspl,dspl)
          do je=0,mj
            xx = r(je)
            vzt_eqp(je) = seval(ns0,xx,rho_e,tor_rot_vel_e,bspl,cspl,dspl)*1.e+5/va0
          end do 				  
          call spline(ns0,rho_e,pol_rot_vel_e,bspl,cspl,dspl)
          do je=0,mj
            xx = r(je)
            vth_eqp(je) = seval(ns0,xx,rho_e,pol_rot_vel_e,bspl,cspl,dspl)*1.e+5/va0
          end do 
	 end if
	  ...
	  etactte = 0.02116*uion_e*uion_e*coul_log*qe*qe*sqrt(me)/(epsil*epsil*sqrt(kblotz*kblotz*kblotz))  !! ctte resistivity
          ...
          do je=0,mj		
		    ...
		    if(Eq_vel_on .eq. 0) then
                                        vzt_eq(je)=vzt_eqp(je)
		    end if  
		    if(Eq_velp_on .eq. 0) then
